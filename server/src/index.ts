...
async function runGlobalSync(env: Bindings) {
  ...
  try {
    ...
    try {
        const gdeltUrl = `https://api.gdeltproject.org/api/v2/doc/doc?query=(military%20OR%20diplomacy%20OR%20geopolitics%20OR%20conflict)&mode=artlist&maxrecords=10&format=json&sort=datedesc`
